...
const BurgerIngredients = () => {
    return (
        <div className={styles.root}>
            ...
            {[['bun', 'Булки'], ['sauce', 'Соусы'], ['main', 'Начинки']].map((type, ix) => {
                return <section className={styles.tabSection} key={ix}>
                    <h2 className={styles.tabSectionHeader}>{type[1]}</h2>
                    {data.filter(item => item.type === type[0]).map((ingredient, index) => {
                        return <div className={styles.Ingredient} key={index+1}>
